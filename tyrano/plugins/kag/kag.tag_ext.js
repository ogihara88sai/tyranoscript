...
frame_image     = パーツアニメーションの差分画像。カンマ区切りで複数指定できます。`storage`に指定した画像と拡張子が同じである場合、拡張子を省略して書くことができます。（アニメーションに使用する画像は、storageに指定した画像と同じ場所に配置します。アニメーションに使用する画像のファイル名が空白記号から始まっていてはいけません）,
frame_time      = パーツアニメーションの各コマの時間。ミリ秒単位の数値をカンマ区切りで記述します。storageに指定した画像の分も含めて指定します。ハイフンで区切った2個1組の数値を指定することで、その範囲からランダムで決定されるようにできます。目パチの時間にバラつきをもたせたい場合に使用できます。省略した場合、標準的な目パチの頻度・速度を想定し、4000-8000,50,50,50…が設定されます。,
frame_direction = パーツアニメーションの最後のコマを表示したあと、すぐ最初のコマに戻るのではなく逆順にコマを表示していく（0→1→2→1→0）ようにしたい場合、`alternate`を指定します。,
frame_loop      = アニメーションをループさせずに最後のコマで止めたい場合は`false`を指定します。,
lip_image      = 口パクに使用する画像ファイル。`frame_image`と同じように指定します。なお、`frame_image`と`lip_image`を併用することはできません。音声ファイルによるリップシンクを行う場合には、より口が閉じている画像から順に書きます。,
lip_time       = 口パクの画像を切り替える時間をミリ秒単位で指定します。省略した場合は50ミリ秒となります。,
lip_type       = 音声ファイル（ボイス）によるリップシンクを行いたい場合は`voice`を指定します。省略した場合はテキストベースの口パクになります。
lip_volume     = 音声ファイルによるリップシンクを行う場合に、画像を切り替える音量の区切りをカンマ区切りの数値で指定します。音量は音声ファイル再生中にリアルタイムに解析され、0～100で表されます。数値は`lip_image`で指定した画像の数だけ必要です。このパラメータは省略してもかまいませんが、違和感がある場合は微調整することを推奨します。,
lip_se_buf     = リップシンクの対象とする`[playse]`の`buf`を指定します。ここで指定した`buf`で`[playse]`が実行されたとき、`chara_ptext`にこのキャラの名前が入っているなら、このキャラのリップシンクを開始します。,
lip_se_buf_all = リップシンクの対象とする`[playse]`の`buf`を指定します。ここで指定された`buf`で実行されたすべての`[playse]`について、このキャラのリップシンクを開始します。（chara_ptextに関係なく）
